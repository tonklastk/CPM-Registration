...
- PostgreSQL health monitoring and transactional updates
...
TRANSLATIONS = {
    "English": {
        "app_subtitle": "Plant Access Management System", "secure": "SECURED WORKFLOW",
        ...
        "visitor_information": "Visitor information", "full_name": "Full name *", "company": "Company / organization *", "email": "Email address *", "visit_date": "Requested visit date *", "leave_date": "Leaving date *", "leave_date_label": "Leaving date",
        ...
        "safety_text": "I confirm that the visitor(s) currently have no symptoms of communicable illnesses (fever, respiratory illness, skin infections) and agree to adhere strictly to all GMP gowning, safety and security policies of the plant. *",
        ...
        "specify_reason": "Please specify the reason for visiting.", "accept_safety": "The safety declaration must be accepted before submitting.", "past_date": "The visit date cannot be in the past.", "invalid_leave_date": "The leaving date cannot be earlier than the requested visit date.",
        ...
        "monitoring": "Request monitoring", "status_filter": "Status", "all": "All", "search": "Search", "search_monitor_placeholder": "Name, company, email...", "no_matching": "No matching requests.", "audit": "Audit trail", "approval_log": "Approval log export", "download_csv": "Download approval log (.csv)", "download_xlsx": "Download approval log (.xlsx)", "database_status": "PostgreSQL database", "database_online": "Connected", "database_offline": "Unavailable", "database_latency": "Connection latency", "maintenance_title": "System under maintenance", "maintenance_message": "The PostgreSQL database server is currently unavailable. The Plant Access Management System cannot be used until the database connection is restored.", "maintenance_hint": "Please contact the system administrator or try again after maintenance is completed.", "database_error": "Database initialization failed.",
    ...
    "ไทย": {
        "app_subtitle": "ระบบบริหารจัดการการเข้า-ออกโรงงาน", "secure": "SECURED WORKFLOW",
        "visitor_tab": "ลงทะเบียนผู้มาติดต่อ", "approval_tab": "ระบบอนุมัติ", "admin_tab": "ผู้ดูแลระบบ",
        "visitor_title": "ระบบขออนุญาตเข้าพื้นที่โรงงาน", "visitor_subtitle": "ระบบขออนุญาตเข้าสู่พื้นที่โรงงานที่ปลอดภัย ตรวจสอบย้อนกลับได้ สำหรับผู้มาติดต่อ ผู้รับเหมา และคู่ค้า",
        "approval_title": "ระบบอนุมัติ", "approval_subtitle": "ตรวจสอบคำขอ เอกสารประกอบ และประวัติการอนุมัติได้ในที่เดียว",
        "admin_title": "ผู้ดูแลระบบ", "admin_subtitle": "จัดการบัญชีผู้อนุมัติ การส่งอีเมล และกิจกรรมของระบบ",
        "visitor_kicker": "สำหรับผู้มาติดต่อ", "approval_kicker": "พื้นที่ทำงานสำหรับผู้อนุมัติ", "admin_kicker": "การดูแลระบบ",
        ...
        "visitor_information": "ข้อมูลผู้มาติดต่อ", "full_name": "ชื่อ-นามสกุล *", "company": "บริษัท / หน่วยงาน *", "email": "อีเมล *", "visit_date": "วันที่ขอเข้าพบ *", "leave_date": "วันที่ออกจากโรงงาน *", "leave_date_label": "วันที่ออกจากโรงงาน",
        "visit_details": "รายละเอียดการเข้ามาติดต่อ", "reason": "วัตถุประสงค์การเข้ามาติดต่อ *", "zones": "พื้นที่ที่ต้องการเข้ามาติดต่อ *", "other_reason": "โปรดระบุวัตถุประสงค์ *", "other_placeholder": "ระบุวัตถุประสงค์การเข้าพบ",
        "documents": "เอกสารประกอบ", "upload": "อัปโหลด PDF / JPG / PNG / DOCX (ขนาดไม่เกิน {mb} MB)", "safety": "คำรับรองด้านความปลอดภัยและข้อกำหนด GMP",
        "safety_text": "ข้าพเจ้ารับทราบและยืนยันว่าผู้ขอเข้าพื้นที่ไม่มีโรคติดต่อ (ไข้, อาการทางระบบทางเดินหายใจ, การติดเชื้อทางผิวหนัง) และตกลงปฏิบัติตามข้อกำหนดของ GMP รวมถึงนโยบายความปลอดภัยของโรงงานอย่างเคร่งครัด *",
        "submit": "ส่งคำขอเข้าโรงงาน", "required": "กรุณากรอกข้อมูลผู้มาติดต่อและพื้นที่ที่ต้องการเข้าให้ครบถ้วน", "valid_email": "กรุณากรอกอีเมลให้ถูกต้อง", "specify_reason": "กรุณาระบุวัตถุประสงค์การเข้าพบ", "accept_safety": "ต้องยอมรับคำรับรองด้านความปลอดภัยก่อนส่งคำขอ", "past_date": "วันที่เข้าพบต้องไม่เป็นวันที่ผ่านมาแล้ว", "invalid_leave_date": "วันที่ออกจากโรงงานต้องไม่ก่อนวันที่ขอเข้าพบ",
        ...
        "monitoring": "ติดตามคำขอ", "status_filter": "สถานะ", "all": "ทั้งหมด", "search": "ค้นหา", "search_monitor_placeholder": "ชื่อ บริษัท อีเมล...", "no_matching": "ไม่พบคำขอที่ตรงกับเงื่อนไข", "audit": "บันทึกการตรวจสอบ", "approval_log": "ส่งออกบันทึกการอนุมัติ", "download_csv": "ดาวน์โหลดบันทึกการอนุมัติ (.csv)", "download_xlsx": "ดาวน์โหลดบันทึกการอนุมัติ (.xlsx)", "database_status": "ฐานข้อมูล PostgreSQL", "database_online": "เชื่อมต่อแล้ว", "database_offline": "ไม่พร้อมใช้งาน", "database_latency": "เวลาในการเชื่อมต่อ", "maintenance_title": "ระบบอยู่ระหว่างการบำรุงรักษา", "maintenance_message": "ขณะนี้ไม่สามารถเชื่อมต่อเซิร์ฟเวอร์ฐานข้อมูล PostgreSQL ได้ ระบบบริหารจัดการการเข้า-ออกโรงงานจึงไม่สามารถใช้งานได้จนกว่าการเชื่อมต่อฐานข้อมูลจะกลับมาเป็นปกติ", "maintenance_hint": "กรุณาติดต่อผู้ดูแลระบบหรือลองใช้งานอีกครั้งหลังการบำรุงรักษาเสร็จสิ้น", "database_error": "ไม่สามารถเริ่มต้นฐานข้อมูลได้",
...
def apply_liquid_glass_ui() -> None:
    st.markdown(
        ...
        /* Fully hide the top black bar/header while preserving interactivity of internal items */
        ...
        /* Ensure sidebar toggle remains clickable if header gets obscured */
        [data-testid="stSidebarCollapsedControl"] {
            z-index: 999999 !important;
        }

        /* Hide deploy button without killing the toolbar */
        .stAppDeployButton {
        ...
        [data-testid="stSidebar"] { background: rgba(8,15,28,.72) !important; border-right:1px solid rgba(255,255,255,.10); }
        [data-testid="stSidebarContent"] { padding:22px 16px 18px !important; }
        .sidebar-brand { display:flex; align-items:center; gap:12px; padding:12px 10px 20px; }
        .sidebar-brand-icon { width:42px; height:42px; display:flex; align-items:center; justify-content:center; border-radius:14px; background:linear-gradient(135deg,rgba(105,168,255,.35),rgba(141,125,255,.22)); border:1px solid rgba(255,255,255,.14); }
        .sidebar-brand-title { font-weight:800; color:#fff; font-size:1rem; }
        .sidebar-brand-subtitle { color:#98a8bf; font-size:.72rem; margin-top:2px; line-height:1.25; }
        .sidebar-section-title { color:#7f8fa8; font-size:.68rem; font-weight:800; letter-spacing:.12em; margin:8px 8px; }
        .sidebar-divider { height:1px; background:rgba(255,255,255,.10); margin:16px 4px; }
        .sidebar-footer { margin:18px 8px 0; color:#62dc91; font-size:.76rem; font-weight:700; }
        
        /* Custom radio button styles (Navigation tabs) */
        [data-testid="stSidebar"] [data-testid="stRadio"] > div { gap:7px !important; }
        
        [data-testid="stSidebar"] [data-testid="stRadio"] div[role="radiogroup"] label { 
            min-height:48px !important; height:48px !important; box-sizing:border-box !important; padding:0 14px !important; margin:0 !important; 
            border:1px solid rgba(255,255,255,.10) !important; border-radius:16px !important; background:rgba(255,255,255,.045) !important; color:#aebbd0 !important; 
            display:flex !important; align-items:center !important; justify-content:center !important; cursor: pointer !important;
        }
        
        [data-testid="stSidebar"] [data-testid="stRadio"] div[role="radiogroup"] label:hover { 
            background:rgba(255,255,255,.085) !important; border-color:rgba(255,255,255,.17) !important; 
        }
        
        [data-testid="stSidebar"] [data-testid="stRadio"] div[role="radiogroup"] label:has(input:checked) { 
            color:#fff !important; background:linear-gradient(135deg,rgba(105,168,255,.34),rgba(141,125,255,.24)) !important; 
            border-color:rgba(255,255,255,.20) !important; box-shadow:inset 0 1px 0 rgba(255,255,255,.13),0 8px 22px rgba(40,90,170,.16); 
        }
        
        /* --- Bulletproof hiding of the radio dot --- */
        [data-testid="stSidebar"] [data-testid="stRadio"] div[role="radiogroup"] label input[type="radio"] { 
            display:none !important; 
        }
        /* Hides the adjacent div which represents the physical circle marker */
        [data-testid="stSidebar"] [data-testid="stRadio"] div[role="radiogroup"] label input[type="radio"] + div { 
            display:none !important; 
        }
        
        /* Ensure the text centers correctly and fills space */
        ...
        [data-testid="stSidebar"] [data-testid="stRadio"] div[role="radiogroup"] label p { 
            margin:0 !important; text-align:center !important; font-weight:700 !important; line-height:48px !important; color: inherit !important; 
        }
        ...
        .db-health-dot { width:10px; height:10px; border-radius:999px; flex:0 0 10px; box-shadow:0 0 14px currentColor; }
        .db-health.online { color:#55d98b; }
        .db-health.offline { color:#ff6b81; }
        .db-health strong { color:#f7f9fc; }
        .db-health-detail { color:#9facbd; font-size:.78rem; margin-top:2px; overflow-wrap:anywhere; }

        [data-testid="stHorizontalBlock"] { gap:clamp(.65rem,1.6vw,1.25rem) !important; }
        [data-testid="stTabs"] [data-baseweb="tab-list"] { overflow-x:auto; scrollbar-width:none; gap:.35rem; }
        [data-testid="stTabs"] [data-baseweb="tab-list"]::-webkit-scrollbar { display:none; }
        [data-testid="stDataFrame"] { max-width:100%; }

        @media (max-width: 900px) {
            .block-container { padding:1.05rem .85rem 3rem !important; max-width:100% !important; }
            .hero { padding:21px 19px; border-radius:22px; margin-bottom:16px; }
            .hero-title { font-size:clamp(1.65rem,8vw,2.35rem); }
            .hero-subtitle { font-size:.94rem; line-height:1.5; }
            [data-testid="stHorizontalBlock"] { flex-wrap:wrap !important; }
            [data-testid="column"] { flex:1 1 100% !important; width:100% !important; min-width:100% !important; }
            [data-testid="stForm"], [data-testid="stExpander"], [data-testid="stMetric"] { border-radius:18px !important; }
            .stButton > button, .stFormSubmitButton > button, .stDownloadButton > button { min-height:48px !important; width:100% !important; }
            [data-testid="stSidebar"] { width:min(88vw,330px) !important; }
            [data-testid="stSidebarContent"] { padding:16px 12px 18px !important; }
            .app-topbar { align-items:flex-start; }
            .status-pill, .db-health { width:100%; box-sizing:border-box; }
            .status-pill { text-align:center; }
        }
        @media (max-width: 520px) {
            .block-container { padding-left:.65rem !important; padding-right:.65rem !important; }
            .hero { padding:18px 16px; border-radius:19px; }
            .sidebar-brand { padding-left:6px; padding-right:6px; }
            input, textarea, div[data-baseweb="select"] > div { font-size:16px !important; }
        }
        </style>
        """,
...
def database_health() -> tuple[bool, str, float]:
    """Return PostgreSQL connectivity state, database name and connection latency in ms."""
    ...
    try:
        ...
        with psycopg.connect(
            host=cfg["host"], port=cfg["port"], dbname=cfg["database"],
            user=cfg["user"], password=cfg["password"], sslmode=cfg["sslmode"],
            connect_timeout=cfg["connect_timeout"], row_factory=dict_row, autocommit=True,
        ) as conn:
            row = conn.execute("SELECT current_database() AS database_name").fetchone()
        latency_ms = (time.perf_counter() - started) * 1000
        return True, str(row["database_name"] if row else cfg["database"]), latency_ms
    except Exception:
        latency_ms = (time.perf_counter() - started) * 1000
        return False, "PostgreSQL connection unavailable", latency_ms


def render_database_maintenance_notice() -> None:
    """Block the application when PostgreSQL cannot be reached."""
    render_hero(tr("maintenance_title"), tr("maintenance_message"), "DATABASE OFFLINE")
    if hasattr(st, "dialog"):
        @st.dialog(tr("maintenance_title"))
        def _maintenance_dialog() -> None:
            st.error(tr("maintenance_message"))
            st.caption(tr("maintenance_hint"))
        _maintenance_dialog()
    else:
        st.error(tr("maintenance_message"))
        st.caption(tr("maintenance_hint"))
...
def init_db() -> None:
    with db_connection() as conn:
        ...
        """)
        # Backward-compatible migration for databases created before leaving dates.
        conn.execute("ALTER TABLE access_requests ADD COLUMN IF NOT EXISTS leave_date TEXT")
        conn.execute("UPDATE access_requests SET leave_date=visit_date WHERE leave_date IS NULL OR leave_date='' ")
        conn.execute("ALTER TABLE access_requests ALTER COLUMN leave_date SET NOT NULL")
        ...
        conn.execute("CREATE INDEX IF NOT EXISTS idx_access_leave_date ON access_requests(leave_date)")
...
def approval_log_dataframe(limit: int = 10000) -> pd.DataFrame:
    """Return approval/rejection events enriched with visitor/request details."""
    with db_connection() as conn:
        rows = conn.execute(
            """
            SELECT
                a.timestamp, a.actor, a.action, a.details, a.request_id,
                r.visitor_name, r.visitor_email, r.company,
                r.visit_date, r.leave_date, r.status, r.confirmation_number
            FROM audit_log a
            LEFT JOIN access_requests r ON r.id = a.request_id
...
def dataframe_to_xlsx_bytes(df: pd.DataFrame, sheet_name: str = "Approval Log") -> bytes:
    output = BytesIO()
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        ...
        for column_cells in worksheet.columns:
            ...
            worksheet.column_dimensions[column_cells[0].column_letter].width = max(10, width)
...
def generate_ticket_pdf(
    ...
    add_row("Leaving date", leave_date)
    ...
    with db_connection() as conn:
        conn.execute(
            ...
            INSERT INTO access_requests (
                id, visitor_name, visitor_email, company, visit_date, leave_date,
...
def list_requests(
    ...
    if search.strip():
        ...
        sql += """
            AND (
                visitor_name ILIKE %s
                OR visitor_email ILIKE %s
                OR company ILIKE %s
                OR confirmation_number ILIKE %s
            )
        """
...
def approve_request(request_id: str, role: str, actor: str) -> tuple[bool, str]:
    """
    Uses PostgreSQL row-level locking so simultaneous browser sessions cannot
    approve the same request in an inconsistent state.
    """
        ...
        if updated:
            ...
            send_email(
                ...
                (
                    ...
                    f"Your approved access period is {updated['visit_date']} to {updated['leave_date']}.\n\n"
        ...
        if updated:
            send_email(
                ...
                (
                    ...
                    f"Leaving date: {updated['leave_date']}\n"
...
def reject_request(
    ...
    send_email(
        ...
        (
            ...
            f"Your request for access from {row['visit_date']} to {row['leave_date']} has been rejected.\n\n"
            f"Reason: {reason}\n\n"
...
def render_request_details(row: dict, show_actions: bool, role: str = ""):
    ...
    c3.write(f"**{tr('email_label')}**  \n{row['visitor_email']}")
    c1, c2, c3 = st.columns(3)
    c1.write(f"**{tr('visit_date_label')}**  \n{row['visit_date']}")
    c2.write(f"**{tr('leave_date_label')}**  \n{row['leave_date']}")
    c3.write(f"**{tr('status')}**  \n{status_label(row['status'])}")
    c1, c2 = st.columns(2)
    c1.write(f"**{tr('reason_label')}**  \n{row['reason']}")
    c2.write(f"**{tr('zones_label')}**  \n{row['areas_needed']}")
    st.write(f"**{tr('health')}:** {row['health_declaration']}")
    ...
    if row["document_path"] and Path(row["document_path"]).exists():
        ...
        st.download_button(
            ...
            file_name=f"Document_{row['id'][-8:]}{path.suffix}",
    ...
    reject_reason = st.text_input(
        tr("rejection_reason"),
        key=f"rejection_{row['id']}",
        placeholder=tr("rejection_placeholder"),
    )
    a, b = st.columns(2)
    with a:
        if st.button(tr("approve"), key=f"approve_{row['id']}", use_container_width=True):
            ok, message = approve_request(
                row["id"], role, actor=st.session_state.get("approver_email", "unknown")
            )
            if ok:
                st.success(message)
                time.sleep(.25)
    ...
    with b:
        if st.button(tr("reject"), key=f"reject_{row['id']}", use_container_width=True):
            if not reject_reason.strip():
                st.error(tr("enter_rejection"))
            else:
                ok, message = reject_request(
                    row["id"], st.session_state.get("approver_email", "unknown"), reject_reason
                )
                if ok:
                    st.success(message)
                    time.sleep(.25)
...
def visitor_registration_tab() -> None:
    ...
    with st.form("registration_form", clear_on_submit=True):
        ...
        with c1:
            ...
            company = st.text_input(tr("company"))
        with c2:
            visitor_email = st.text_input(tr("email"))
            visit_date = st.date_input(tr("visit_date"), min_value=date.today())
            leave_date = st.date_input(tr("leave_date"), value=visit_date, min_value=visit_date)
    ...
    clean_email = visitor_email.strip()
    clean_company = company.strip()
    if not clean_name or not clean_email or not clean_company or not areas:
        st.error(tr("required")); return
    if not valid_email(clean_email):
        st.error(tr("valid_email")); return
    if reason_selection == "Other" and not specified_reason.strip():
        st.error(tr("specify_reason")); return
    if not health_check:
        st.error(tr("accept_safety")); return
    if visit_date < date.today():
        st.error(tr("past_date")); return
    if leave_date < visit_date:
        st.error(tr("invalid_leave_date")); return

    request_id = str(uuid.uuid4())
    doc_path = ""
    try:
        if uploaded_file:
            doc_path = save_uploaded_file(uploaded_file, request_id)
        ...
        with db_connection() as conn:
            conn.execute(
                """
                INSERT INTO access_requests (
                    id,visitor_name,visitor_email,company,visit_date,leave_date,reason,
                    areas_needed,health_declaration,document_path,status,app1_status,
                    app2_status,final_status,rejection_reason,timestamp,confirmation_number
                ) VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)
                """,
                (
                    request_id, clean_name, clean_email, clean_company,
                    visit_date.isoformat(), leave_date.isoformat(), reason, ", ".join(areas),
                    "Accepted", doc_path, "Pending Initial", "Pending", "Pending", "Pending",
                    "", datetime.now().isoformat(timespec="seconds"), "",
                ),
            )

        audit(
            request_id, clean_email, "REQUEST_CREATED",
            f"Access: {visit_date.isoformat()} to {leave_date.isoformat()} | Zones: {', '.join(areas)}",
        )
        body = (
            "ACTION REQUIRED: New Plant Access Request\n\n"
            f"Visitor: {clean_name}\n"
            f"Company: {clean_company}\n"
            f"Email: {clean_email}\n"
            f"Requested visit date: {visit_date.isoformat()}\n"
            f"Leaving date: {leave_date.isoformat()}\n"
            f"Reason: {reason}\n"
            f"Zones: {', '.join(areas)}\n"
        ...
        for recipient in (get_setting("approver1"), get_setting("approver2")):
            if recipient:
                email_results.append(
                    send_email(recipient, f"APPROVAL REQUIRED: Access Request - {clean_name}", body)
                )
        st.success(tr("submitted"))
        st.code(request_id, language="text")
        if email_results and not all(email_results):
            st.warning(tr("stored_email_warning"))
    except Exception as exc:
        if doc_path:
            try:
...
def approver_dashboard_tab() -> None:
    ...
    with pending_tab:
        ...
        for row in rows:
            title=f"{row['visitor_name']}  •  {row['company']}  •  {row['visit_date']} → {row['leave_date']}"
        ...
        if not approved_rows:
            ...
            for row in approved_rows:
                with st.container(border=True):
                    ...
                    b.write(f"**{tr('leave_date_label')}:** {row['leave_date']}")
                    c.write(f"**{tr('visitor_email')}:** {row['visitor_email']}")
                    c.write(f"**{tr('zones_requested')}:** {row['areas_needed']}")
                    st.caption(tr("arrival_note"))
                    pass_path = GENERATED_DIR / f"Access_Ticket_{row['confirmation_number']}.pdf"
                    if not pass_path.exists() and row["confirmation_number"]:
                        try:
                            pass_path = generate_ticket_pdf(
                                row["visitor_name"], row["visit_date"], row["leave_date"],
                                row["reason"], row["areas_needed"], row["company"],
                    ...
                    if pass_path.exists():
                        st.download_button(
                            tr("view_pass"), data=pass_path.read_bytes(), file_name=pass_path.name,
                            key=f"approved_pass_{row['id']}", use_container_width=True,
                        )
...
def admin_dashboard_tab() -> None:
    ...
    if not st.session_state.get("admin_logged_in", False):
        with st.form("admin_login"):
            password = st.text_input(tr("admin_password"), type="password")
            submitted = st.form_submit_button(tr("admin_signin"), use_container_width=True)
        if submitted:
            if authenticate_admin(password):
                st.session_state["admin_logged_in"] = True
                st.rerun()
            else:
                st.error(tr("invalid_admin"))
        return

    db_ok, db_name, db_latency = database_health()
    top1, top2 = st.columns([8, 2])
    top1.success(tr("admin_active"))
    if top2.button(tr("signout"), key="admin_logout", use_container_width=True):
        st.session_state["admin_logged_in"] = False
        st.rerun()

    state_class = "online" if db_ok else "offline"
    state_text = tr("database_online") if db_ok else tr("database_offline")
    st.markdown(
        f"""
        <div class="db-health {state_class}">
            <span class="db-health-dot" style="background:currentColor"></span>
            <div>
                <strong>{tr('database_status')}: {state_text}</strong>
                <div class="db-health-detail">
                    {db_name if db_ok else tr('maintenance_message')} · {tr('database_latency')}: {db_latency:.0f} ms
                </div>
            </div>
        </div>
        """,
    ...
    for user in get_users():
        ...
        with st.container(border=True):
            ...
            if st.button(tr("save_account"), key=f"save_{role}", use_container_width=True):
                try:
                    update_user(role, new_email, new_password or None, active)
                    audit(None, "admin", "USER_UPDATED", f"{role}: {new_email}")
                    st.success(tr("updated", label=label)); st.rerun()
                except Exception as exc:
                    st.error(str(exc))

    st.divider(); st.subheader(tr("smtp")); st.info(tr("smtp_info"))
    cfg = smtp_config()
    st.write(f"**{tr('server')}:** {cfg['host']}:{cfg['port']}")
    st.write(f"**{tr('sender')}:** {cfg['username'] or tr('not_configured')}")
    test_recipient = st.text_input(tr("test_email_to"), value=cfg["username"] or "")
    if st.button(tr("send_test"), use_container_width=True):
        if not valid_email(test_recipient):
            st.error(tr("enter_valid_recipient"))
        else:
            ok = send_email(test_recipient, "Plant Access System - SMTP Test", "This is a test email from the Plant Access Management System.")
            if ok:
                audit(None, "admin", "SMTP_TEST", test_recipient); st.success(tr("test_success"))

    st.divider(); st.subheader(tr("monitoring"))
    counts = dashboard_counts(); c1, c2, c3, c4 = st.columns(4)
    c1.metric(tr("awaiting_initial"), counts["Pending Initial"])
    c2.metric(tr("awaiting_final"), counts["Pending Final"])
    c3.metric(tr("approved"), counts["Approved"])
    c4.metric(tr("rejected"), counts["Rejected"])
    c1, c2 = st.columns(2)
    with c1:
        status_display = st.selectbox(tr("status_filter"), [tr("all")] + [status_label(x) for x in STATUS_LABELS.keys()])
    with c2:
        search = st.text_input(tr("search"), placeholder=tr("search_monitor_placeholder"))
    reverse_status = {status_label(x): x for x in STATUS_LABELS.keys()}
    status_filter = reverse_status.get(status_display, "All")
    rows = list_requests(status_filter, search, limit=1000)
    if rows:
        display = pd.DataFrame([
            {
                tr("visitor"): r["visitor_name"], tr("company_label"): r["company"],
                tr("visit_date_label"): r["visit_date"], tr("leave_date_label"): r["leave_date"],
                tr("status"): status_label(r["status"]), tr("initial_1"): status_label(r["app1_status"]),
                tr("initial_2"): status_label(r["app2_status"]), tr("final_approver"): status_label(r["final_status"]),
                tr("arrival_confirm"): r["confirmation_number"], tr("submitted_at", time=""): r["timestamp"],
            }
            for r in rows
        ])
        st.dataframe(display, hide_index=True, use_container_width=True)
    else:
        st.info(tr("no_matching"))
    ...
    st.markdown(f"### {tr('approval_log')}")
    approval_df = approval_log_dataframe()
    export_c1, export_c2 = st.columns(2)
    export_c1.download_button(
        tr("download_csv"), data=approval_df.to_csv(index=False).encode("utf-8-sig"),
        file_name=f"approval_log_{date.today().isoformat()}.csv", mime="text/csv", use_container_width=True,
    )
    try:
        xlsx_bytes = dataframe_to_xlsx_bytes(approval_df)
        export_c2.download_button(
            tr("download_xlsx"), data=xlsx_bytes,
            file_name=f"approval_log_{date.today().isoformat()}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True,
        )
    except ImportError:
        export_c2.warning("Install openpyxl to enable Excel export: pip install openpyxl")


# ============================================================
# 13. STABLE SIDEBAR NAVIGATION
# ============================================================

def render_sidebar_navigation() -> str:
    """Stable dashboard navigation using Streamlit's native sidebar."""
    if "dashboard" not in st.session_state:
        st.session_state["dashboard"] = "visitor"
    if "language" not in st.session_state:
        st.session_state["language"] = "English"
    labels = {"visitor": tr("visitor_tab"), "approval": tr("approval_tab"), "admin": tr("admin_tab")}
    ids = ["visitor", "approval", "admin"]
    with st.sidebar:
        st.markdown(f"""<div class=\"sidebar-brand\"><div class=\"sidebar-brand-icon\">🏭</div><div><div class=\"sidebar-brand-title\">Plant Access</div><div class=\"sidebar-brand-subtitle\">{tr("app_subtitle")}</div></div></div>""", unsafe_allow_html=True)
        st.markdown('<div class="sidebar-section-title">NAVIGATION</div>', unsafe_allow_html=True)
        selected = st.radio("Dashboard", ids, index=ids.index(st.session_state["dashboard"]), format_func=lambda x: labels[x], key="dashboard_navigation", label_visibility="collapsed")
        if selected != st.session_state["dashboard"]:
            st.session_state["dashboard"] = selected
            st.rerun()
        st.markdown('<div class="sidebar-divider"></div>', unsafe_allow_html=True)
        st.markdown(f'<div class="sidebar-section-title">{tr("language")}</div>', unsafe_allow_html=True)
        languages = ["English", "ไทย"]
        selected_language = st.selectbox(tr("language"), languages, index=languages.index(st.session_state["language"]), key="language_selector_sidebar", label_visibility="collapsed")
        if selected_language != st.session_state["language"]:
            st.session_state["language"] = selected_language
            st.rerun()
        st.markdown(f'<div class="sidebar-footer">● {tr("secure")}</div>', unsafe_allow_html=True)
...
def main() -> None:
    ...
    db_ok, _, _ = database_health()
    if not db_ok:
        render_database_maintenance_notice()
        st.stop()
    ...
    try:
        if dashboard == "visitor":
            visitor_registration_tab()
        elif dashboard == "approval":
